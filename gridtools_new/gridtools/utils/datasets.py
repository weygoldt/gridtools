...
class WTData:
    ...
    def _check_health(self):
        ...
        if time_length < np.max(self.track_indices):
            raise GridDataMismatch("Index vector contains indices larger than time vector!")
        else:
            logger.info("Passed check: Index vector does not contain indices larger than time vector.")
        ...
        if power_electrode_count == grid_electrode_count:
            logger.info("Passed check: Power vector has the number of electrodes as the grid.")
        else:
            raise GridDataMismatch("Power vector has not the same number of electrodes as the grid!")
...
class WTProcessed(WTData):
    ...
    def _check_health(self):
        if self.temperature.shape[0] != self.light.shape[0]:
            raise GridDataMismatch("Temperature vector has not the same length as light vector!")
        else:
            logger.info("Passed check: Temperature vector has the same length as light vector.")
        ...
        if self.x.shape[0] == self.y.shape[0] == self.track_freqs:
            logger.info("Passed check: All positions have the same length as tracked vectors.")
        else:
            raise GridDataMismatch("Not all positions have the same length as tracked vectors!")
