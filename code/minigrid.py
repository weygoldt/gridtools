...
def make_eod():
    ...
    rise_positions = [
        random.randint(10, length - 10 - len(rise_lenght)) for i in range(rise_number)
    ]
    rise = rise_height * np.exp(-rise_lenght / 5)

    # add rises
    for i in range(rise_number):
        baseline[rise_positions[i] : rise_positions[i] + len(rise_lenght)] += rise
...
def grid2block(file, grid1, number):
    ...
    freq = block.create_data_array("eod", "tracked eod", data=grid1.frequencies)
    xpos = block.create_data_array("xpos", "x coordinates", data=grid1.xpositions)
    ypos = block.create_data_array("ypos", "y coordinates", data=grid1.ypositions)
    ident = block.create_data_array(
