...
from .functions import find_closest, is_odd, kde1d, velocity2d
from .termcolors import TermColor


class GridTracks:
    """
    Class to load, process and save electrode grid data as it is generated by the wavetracker algorithm (https://github.com/tillraab/wavetracker). Processing functions can be used independently but some may rely on others.

    Example: If `EODsorter.py` from the wavetracker repository was used to fix some
    frequency traces on the spectrogram, the power array `sign_v.npy` must be recalculated from the raw grid data using the function `fill_powers()` before e.g.
    position estimation can be performed.
    """
    ...
    def __init__(self, datapath, finespec=False, verbose=True):
        ...
        try:
            ...
            print(
                TermColor.warn("[ GridTracks.__init__ ]")
    ...
    def fill_powers(self, filename="traces-grid1.raw"):
        """Compute missing power values in the sign_v that are generated when
        manually editing frequency tracks in the wavetracker GUI (EODsorter.py).
        New power vector is saved and old sign_v.npy is overwritten, after a backup
        is created.

        Args:
            filename (str, optional): Name of raw grid data. Defaults to "traces-grid1.raw".

        Returns:
            self: Class instance with new sign_v
        """

        if len(self.ident_v) != len(self.sign_v):
            print(
                TermColor.err("[ GridTracks.fill_powers ]")
        ...
        if self.verbose:
            print(
                TermColor.succ("[ GridTracks.fill_powers ]")
                + " Updating signature vector ..."
            )
        ...
        try:
            ...
            if self.verbose:
                print(
                    TermColor.warn("[ GridTracks.fill_powers ]")
    ...
    def load_logger(self, filename="hobologger.csv"):
        ...
        if len(self.temp) == 0:
            print(
                TermColor.err("[ GridTracks.load_logger ]")
    ...
    def remove_nans(self):
        """Removes all unassigned frequencies (i.e. where 'ident_v' is NAN) from the dataset. Removing NANs is nessecary for position estimation."""
        ...
        print(
            f"{TermColor.GREEN}{TermColor.BOLD}[ GridTracks.remove_nans ]{TermColor.END} Removed unassignes frequencies"
        )
    ...
    def remove_short(self, thresh):
        ...
        for track_id in tqdm(self.ids, disable=self.tqdm_disable):
            ...
            if duration < thresh:
                ...
                try:
                    ...
                    print(
                        TermColor.warn("[ GridTracks.remove_short ]")
        ...
        if indicator_pos:
            print(
                f"{TermColor.YELLOW}{TermColor.BOLD}[ GridTracks.remove_short ]{TermColor.END} No position data found."
            )

        if indicator_pos_smth:
            print(
                f"{TermColor.YELLOW}{TermColor.BOLD}[ GridTracks.remove_short ]{TermColor.END} No smoothed position data found."
            )

        if self.verbose:
            print(
                f"{TermColor.GREEN}{TermColor.BOLD}[ GridTracks.remove_short ]{TermColor.END} Removed {i} short tracks with threshold of {thresh} seconds."
            )
    ...
    def remove_poor(self, thresh):
        ...
        print(
            f"{TermColor.GREEN}{TermColor.BOLD}[ GridTracks.remove_poor ]{TermColor.END} Removed {i} tracks with performance threshold {thresh}"
        )
    ...
    def positions(self, num_el):
        ...
        Args:
            num_el (int): Number of grid electrodes used for position estimation

        Returns:
            self: Class instance containing new arrays xpos and ypos.
        """

        if len(self.ident_v) != len(self.sign_v):
            print(
                TermColor.err("[ GridTracks.positions ]")
        ...
        if self.verbose:
            print(
                f"{TermColor.GREEN}{TermColor.BOLD}[ GridTracks.positions ]{TermColor.END} Computing fish positions from {num_el} electrodes ..."
            )
    ...
    def interpolate(self, positions=True):
        ...
        if (hasattr(self, "xpos") == False) & (all == True):
            print(
                TermColor.err("[ GridTracks.interpolate ]")
            ...
            if self.verbose:
                print(
                    f"{TermColor.GREEN}{TermColor.BOLD}[ GridTracks.interpolate ]{TermColor.END} Interpolating fund_v, sign_v, xpos, ypos..."
                )
            ...
            if positions == False:
                ...
                for track_id in tqdm(self.ids, disable=self.tqdm_disable):
                    ...
                    if test == False:
                        print(
                            f"{TermColor.RED}{TermColor.BOLD}[ GridTracks.interpolate ]{TermColor.END} Frequency array is not the same size as the time array!"
                        )
                    else:
                        pass

    def smooth_positions(self, params):
        """Smooth position coordinate arrays.

        Position cleanup is performed by a combination of velocity filtering, median filtering and a savitzky golay filter.

        Args:
            params: dict containing preprocessing parameters
        """
        ...
        if self.verbose:
            print(
                f"{TermColor.GREEN}{TermColor.BOLD}[ GridTracks.smooth_positions ] {TermColor.END} Smoothing position tracks ..."
            )
    ...
    def check_integrity(self):
        ...
        if len(self.ident_v) != reflen:
            passed = False
            print(
                TermColor.err("[ GridTracks.check_integrity ]")
    ...
    def save(self, outputpath, check=True):
        """Saves all data arrays loaded and processed as well as the raw and smoothed
        position estimates to numpy files in the specified directory.

        Args:
            outputpath (string): The output folder.
        """
        ...
        if len(glob.glob(outputpath + "*.raw")) > 0:
            ...
            if self.verbose:
                print(
                    f"{TermColor.GREEN}{TermColor.BOLD}[ GridTracks.save ]{TermColor.END} Saving data to {outputpath}"
                )
    ...
    def extract_ids(self, ids):
        "Returns a class instance only containing data for the track ids specified in the 'ids' argument. The time array remains untouched, only 'fund_v', 'sign_v'. 'ident_v' and position estimates are truncated."
        ...
        def delete_id_data(self, track_id):
            ...
            if len(self.sign_v) != len(self.ident_v):
                ...
                if self.verbose:
                    print(
                        f"{TermColor.YELLOW}{TermColor.BOLD}[ GridTracks.extract_ids ]{TermColor.END} sign_v and ident_v are not the same length. Skipped signature vector."
                    )
            ...
            try:
                ...
                print(
                    TermColor.warn("[ GridTracks.extract_ids ]")
            ...
            if hasattr(self, "xpos"):
                ...
                if hasattr(self, "xpos_smth"):
                    ...
                    if self.verbose:
                        print(
                            f"{TermColor.YELLOW}{TermColor.BOLD}[ GridTracks.extract_ids ]{TermColor.END} No smoothed position data found."
                        )
            else:
                if self.verbose:
                    print(
                        f"{TermColor.YELLOW}{TermColor.BOLD}[ GridTracks.extract_ids ]{TermColor.END} No smootposition data found."
                    )
            self.ident_v = np.delete(self.ident_v, self.ident_v == track_id)
            self.ids = np.delete(self.ids, self.ids == track_id)
        ...
        if self.verbose:
            print(
                f"{TermColor.GREEN}{TermColor.BOLD}[ GridTracks.extract_ids ]{TermColor.END} Extracting IDs ..."
            )
        for track_id in self.ids:
    ...
    def freq_bandpass(
        ...
        Used particularly for event detection of certain frequency modulations.

        Args:
            rate (float): Sampling rate of time series data
            flow (float): Lower cutoff frequency
            fhigh (float): Higher cutoff frequency
        ...
        if self.verbose:
            print(
                f"{TermColor.GREEN}{TermColor.BOLD}[ GridTracks.freq_bandpass ]{TermColor.END} Applying bandpass filter ..."
            )
    ...
    def plot_freq(self, ax, title="", ids=None, annotate=True):
        """Plots frequency tracks onto provided axis.
        If ids are provided, only the frequency tracks for the respective ids
        are plotted.

        Args:
            ax (_axis_): Matplotlib axis object
            title (str, optional): String for the plot title. Defaults to "".
            ids (list, optional): Ids of interest. Defaults to [] resulting in all.
            annotate (bool, optional): Enable or disable track id annotation.
            Defaults to True.
        """

        if self.verbose:
            print(
                f"{TermColor.GREEN}{TermColor.BOLD}[ GridTracks.plot_frequencies ] {TermColor.END}{title} Plotting frequency tracks ..."
            )
    ...
    def plot_spec(self, ax, title="", ids=None, annotate=True):
        """Plots the spectrogram with overlayed tracked fundamental frequencies
        onto the specified axis. Plots all ids if none are specified.

        Args:
            ax (_axis_): Matplotlib axis object
            spectra (array, optional): Spectrogram data. Defaults to None.
            title (str, optional): String for plot title. Defaults to "".
            ids (list, optional): Track ids to plot. Defaults to [].
            annotate (bool, optional): Enable or disable annotation of track ids.
            Defaults to True.
        """

        if self.verbose:
            print(
                f"{TermColor.GREEN}{TermColor.BOLD}[ GridTracks.plot_spec ] {TermColor.END}{title} Plotting spectrogram ..."
            )
    ...
    def plot_pos(self, ax, title="", ids=None, smoothed=True, legend=True):
        """Plots the triangulated positions of the frequency tracks onto the
        specified axis.

        Args:
            ax (_axis_): Matplotlib axis
            smoothed (bool, optional): Enable or disable smoothing of tracks. Defaults to True.
            ids (list, array, optional): Plots only specified IDs. Defaults to None.
            legend (bool, optional): Enables or disables legend. Defaults to True.
        ...
        if self.verbose:
            print(
                f"{TermColor.GREEN}{TermColor.BOLD}[ GridTracks.plot_pos ] {TermColor.END} Plotting positions ..."
            )
...
class Collective:
    ...
    def __init__(self, gridtracks, ids, thresh=1, verbose=False):
        self.verbose = verbose
        self.ids = ids
        self.monads = [Monad(gridtracks, fish_id) for fish_id in ids]

        # get overlap
        fish_times = [self.monads[i].times.tolist() for i in range(len(ids))]
        time_shared = set(fish_times[0]).intersection(*fish_times[1:])
        ...
        if len(time_shared) > thresh:
            ...
            for i in range(len(ids)):
                self.monads[i].clip(tmin, tmax)
            if self.verbose == True:
                print(
                    TermColor.succ("[ Collective.__init__ ]")
                    + " Collective created succesfully!"
                )
        else:
            self.overlap = False
            print(
                TermColor.err("[ Collective.__init__ ]")
                + " Supplied identities do not share a common overlap!"
            )
    ...
    def plot_pos(self, ax, title="", smoothed=True, legend=True):
        for i, track_id in enumerate(self.ids):
            if smoothed:
                ax.plot(
                    self.monads[i].xpos_smth, self.monads[i].ypos_smth, label=track_id
                )
            else:
                ax.plot(self.monads[i].xpos, self.monads[i].ypos, label=track_id)
    ...
    def plot_freq(self, ax, title="", annotate=True):
        for i, track_id in enumerate(self.ids):
            ax.plot(self.monads[i].times, self.monads[i].fund, label=track_id)
            if annotate:
                ax.annotate(
                    track_id, xy=(self.monads[i].times[0], self.monads[i].fund[0])
                )
        ax.set_title(title, loc="left")
...
class ListRecordings:
    """
    Lists subdiretories of data recordings in a given root directory to iterate over. Directory names specified as strings in the exclude list are ignored (e.g. directories containing metadata, eTermColor.).
    """
    ...
    def __init__(self, path, exclude=[], verbose=False):
        ...
        if self.verbose:
            print(
                TermColor.succ("[ ListRecordings.__init__ ]")
