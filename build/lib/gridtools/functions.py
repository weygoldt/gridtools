...
def kde2d_bwest(x, y, ax, bw_method="silverman"):
    """Computes 2d KDE and approximates bandwidth with silverman or scott method"""
    xy = np.vstack([x, y])
...
def fancy_title(axis, title):
    """Add a title to a matplotlib axis object with a bold first item (e.g. letter, number, etc.) that is separated from the description by a space."""
...
def fade_cmap(cmap):
    """Fades a colormap to transparent. Useful to plot multiple images to the same axes."""
