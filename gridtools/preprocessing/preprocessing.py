...
def remove_unassigned_tracks(data: WTPreprocessing) -> WTPreprocessing:
    ...
    data.track_indices = np.delete(
        data.track_indices, np.isnan(data.track_idents)
    )
    data.track_freqs = np.delete(data.track_freqs, np.isnan(data.track_idents))
    data.track_powers = np.delete(
        data.track_powers, np.isnan(data.track_idents), axis=0
    )
    data.track_idents = np.delete(
...
def remove_short_tracks(
    data: WTPreprocessing, min_length: int
) -> WTPreprocessing:
...
def remove_low_power_tracks(
    data: WTPreprocessing, min_power: float
) -> WTPreprocessing:
...
def remove_poorly_tracked_tracks(
    data: WTPreprocessing, min_coverage: float
) -> WTPreprocessing:
    ...
    for track_id in track(
        data.ids, description="Removing poorly tracked tracks"
    ):
        track_times = data.track_times[
            data.track_indices[data.track_idents == track_id]
        ]
        ...
        if tracking_perf < min_coverage:
            ...
            data.track_freqs = np.delete(
                data.track_freqs, data.track_idents == track_id
            )
            data.track_powers = np.delete(
                data.track_powers, data.track_idents == track_id, axis=0
            )
            data.track_idents = np.delete(
                data.track_idents, data.track_idents == track_id
            )
