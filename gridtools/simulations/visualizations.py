...
def plot_positions(
    ...
    plt.scatter(x=grid[0], y=grid[1], color="grey")
    colors = ["orange", "red", "green", "blue", "purple", "pink", "grey"]
    for i in range(len(original[0])):
        col = colors[i]
        plt.plot(downsampled[0][i], downsampled[1][i], color=col, alpha=1, lw=1.5)
        plt.plot(original[0][i], original[1][i], color=col, alpha=0.5, ls="dashed", lw=1)
        plt.plot(upsampled[0][i], upsampled[1][i], color=col, alpha=0.5, ls="dotted", lw=1)
